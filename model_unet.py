...
class ReconstructiveSubNetwork(nn.Module):
    ...
    def forward(self, x):
        b5 = self.encoder(x)
        output, b2, b3, b4 = self.decoder(b5)
        return output, b2, b3, b4
...
class Unet(nn.Module):
    ...
    def forward(self, x):
        recon_tensor = self.maxpool_layer(x)
        recon_tensor = self.maxpool_layer(recon_tensor)
        recon_tensor = self.maxpool_layer(recon_tensor)
        recon_tensor = self.up_sample(recon_tensor)
        recon_tensor = self.up_sample(recon_tensor)
        recon_tensor = self.up_sample(recon_tensor)
        b5 = self.encoder(recon_tensor)
        output, b2, b3, b4 = self.decoder(b5)
        return output, recon_tensor, b2, b3, b4
